...
export class DataService {
  ...
  constructor(config?: {
    ...
    this.baseUrl =
      config?.baseUrl || import.meta.env.VITE_NOCODB_BASE_URL || "";
    this.apiToken =
      config?.apiToken || import.meta.env.VITE_NOCODB_API_TOKEN || "";
    this.useMockData =
  ...
  async fetchChatRecords(
    ...
    if (!this.baseUrl) {
      console.warn(
        "[DataService] No API URL configured, falling back to mock data",
      );
      console.warn(
        "[DataService] Set VITE_NOCODB_BASE_URL and VITE_NOCODB_API_TOKEN environment variables",
      );
      return this.fetchMockData(s2fId, dateRange);
    }
    ...
    try {
      ...
      console.log(
        "[DataService] API returned records:",
        data.list?.length || 0,
      );
      return data.list || [];
    } catch (error) {
  ...
  calculateAdvancedAnalytics(records: ChatRecord[]) {
    ...
    const bookableChats = records.filter(
      (r) => r.bookable === "Bookable",
    ).length;
    // Fix: use jobId to determine if booked
    const bookedChats = records.filter(
      (r) => r.jobId !== null && r.jobId !== undefined && r.jobId !== "",
    ).length;

    // Conversion Rate = Booked / Bookable (not total chats)
    const conversionRate =
      bookableChats > 0 ? (bookedChats / bookableChats) * 100 : 0;
    const bookableRate =
      totalChats > 0 ? (bookableChats / totalChats) * 100 : 0;
    const bookingRate =
      bookableChats > 0 ? (bookedChats / bookableChats) * 100 : 0;
    ...
    // Average duration
    const durationsWithValue = records.filter(
      (r) => r.duration !== null && r.duration !== undefined,
    );
    const avgDurationSeconds =
      durationsWithValue.length > 0
        ? durationsWithValue.reduce((sum, r) => sum + (r.duration || 0), 0) /
    ...
    const utmSourceMap = new Map<
      string,
      { total: number; bookable: number; booked: number; isPaid: boolean }
    >();
    const utmCampaignMap = new Map<
      string,
      { total: number; bookable: number; booked: number }
    >();
    ...
    records.forEach((record) => {
      ...
      if (session) {
        ...
        if (referrer) {
          ...
          // Check for Precision Door domains (pd or precision in domain)
            ...
            (lowerReferrer.includes("pd") &&
              (lowerReferrer.includes(".com") ||
                lowerReferrer.includes(".net")))
          ) {
        ...
        if (utmSource) {
          ...
          const stats = utmSourceMap.get(sourceKey) || {
            total: 0,
            bookable: 0,
            booked: 0,
            isPaid: hasCampaign,
        ...
        if (utmCampaign) {
          const stats = utmCampaignMap.get(utmCampaign) || {
            total: 0,
            bookable: 0,
            booked: 0,
          };
        ...
        if (referrer && referrer !== "direct") {
          try {
            ...
            const stats = referrerMap.get(domain) || {
              total: 0,
              bookable: 0,
              booked: 0,
            };
        ...
        if (attribution) {
          if (attribution.gclid) {
            const stats = clickIdMap.get("Google Ads") || {
              total: 0,
              bookable: 0,
              booked: 0,
            };
          ...
          if (attribution.fbclid) {
            const stats = clickIdMap.get("Facebook Ads") || {
              total: 0,
              bookable: 0,
              booked: 0,
            };
          ...
          if (attribution.msclkid) {
            const stats = clickIdMap.get("Microsoft Ads") || {
              total: 0,
              bookable: 0,
              booked: 0,
            };
    ...
    const topTrafficSources = Array.from(utmSourceMap.entries())
      .map(([source, stats]) => ({
        ...
        isPaid: stats.isPaid,
        conversionRate:
          stats.bookable > 0 ? (stats.booked / stats.bookable) * 100 : 0,
      }))
    ...
    // === CUSTOMER INSIGHTS ===
    const existingCustomers = records.filter(
      (r) => r.existingCustomer === true,
    ).length;
    const newCustomers = totalChats - existingCustomers;

    // === QUALITY METRICS ===
    const incompleteConversations = records.filter(
      (r) => r.bookable && r.bookable.includes("Incomplete Conversation"),
    ).length;
    ...
    return {
      ...
      directTraffic,
      paidTraffic,
      clickIdBreakdown: Array.from(clickIdMap.entries()).map(
        ([platform, stats]) => ({
          platform,
          count: stats.total,
          bookable: stats.bookable,
